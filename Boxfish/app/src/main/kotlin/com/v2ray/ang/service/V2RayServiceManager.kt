...
object V2RayServiceManager {
    ...
    var serviceControl: SoftReference<ServiceControl>? = null
        set(value) {
            ...
            Libv2ray.initV2Env(
                Utils.userAssetPath(value?.get()?.getService()), Utils.getDeviceIdForXUDPBaseKey()
            )
        }
    ...
    fun startV2rayPoint() {
        ...
        try {
            v2rayPoint.runLoop(settingsStorage.decodeBool(AppConfig.PREF_PREFER_IPV6))
        } catch (e: Exception) {
            Log.d(ANG_PACKAGE, e.toString())
        }
                ...
                AppConfig.MSG_REGISTER_CLIENT -> {
                    if (v2rayPoint.isRunning) {
                        MessageUtil.sendMsg2UI(
                            serviceControl.getService(), AppConfig.MSG_STATE_RUNNING, ""
                        )
                    } else {
                        MessageUtil.sendMsg2UI(
                            serviceControl.getService(), AppConfig.MSG_STATE_NOT_RUNNING, ""
                        )
                    }
                }
    ...
    private fun showNotification() {
        ...
        mBuilder =
            NotificationCompat.Builder(service, channelId).setSmallIcon(R.drawable.ic_boxfish_24)
                .setContentTitle(currentConfig?.remarks)
                .setPriority(NotificationCompat.PRIORITY_MIN).setOngoing(true).setShowWhen(false)
                .setOnlyAlertOnce(true).setContentIntent(contentPendingIntent).addAction(
    ...
    private fun createNotificationChannel(): String {
        val channelId = AppConfig.BOXFISH_CHANNEL_ID
        val channelName = AppConfig.BOXFISH_CHANNEL_NAME
    ...
    private fun updateNotification(contentText: String?, proxyTraffic: Long, directTraffic: Long) {
        if (mBuilder != null) {
            ...
            mBuilder?.setSmallIcon(R.drawable.ic_boxfish_24)

            if (proxyTraffic < NOTIFICATION_ICON_THRESHOLD && directTraffic < NOTIFICATION_ICON_THRESHOLD) {
                mBuilder?.setSmallIcon(R.drawable.ic_boxfish_24)
            } else if (proxyTraffic > directTraffic) {
                mBuilder?.setSmallIcon(R.drawable.ic_boxfish_24) // ic_stat_proxy
            } else {
                mBuilder?.setSmallIcon(R.drawable.ic_boxfish_24) // ic_stat_direct
            }
    ...
    private fun startSpeedNotification() {
        if (mDisposable == null && v2rayPoint.isRunning && settingsStorage.decodeBool(AppConfig.PREF_SPEED_ENABLED)) {
            var lastZeroSpeed = false
            val outboundTags = currentConfig?.getAllOutboundTags()
            outboundTags?.remove(TAG_DIRECT)

            mDisposable = Observable.interval(3, java.util.concurrent.TimeUnit.SECONDS).subscribe {
                ...
                if (!zeroSpeed || !lastZeroSpeed) {
                    ...
                    updateNotification(
                        text.toString().trimEnd(), proxyTotal, directDownlink + directUplink
                    )
                }
