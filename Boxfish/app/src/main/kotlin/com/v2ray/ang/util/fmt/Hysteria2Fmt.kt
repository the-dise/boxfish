...
object Hysteria2Fmt {
    ...
    fun parse(str: String): ServerConfig {
        ...
        if (!queryParam["obfs-password"].isNullOrEmpty()) {
            config.outboundBean?.settings?.obfuscatePassword = queryParam["obfs-password"]
        }
    ...
    fun toUri(config: ServerConfig): String {
        ...
        if (!outbound.settings?.obfuscatePassword.isNullOrEmpty()) {
            dicQuery["obfs"] = "salamander"
            dicQuery["obfs-password"] = outbound.settings?.obfuscatePassword ?: ""
        }
    ...
    fun toNativeConfig(config: ServerConfig, socksPort: Int): Hysteria2Bean? {
        ...
        val obfs = if (outbound.settings?.obfuscatePassword.isNullOrEmpty()) null else
            Hysteria2Bean.ObfuscateBean(
                type = "salamander",
                salamander = Hysteria2Bean.ObfuscateBean.SalamanderBean(
                    password = outbound.settings?.obfuscatePassword
                )
            )
