...
object V2rayConfigUtil {
    ...
    private fun getV2rayNonCustomConfig(context: Context, config: ServerConfig): ConfigResult {
        ...
        v2rayConfig.log.logLevel =
            settingsStorage.decodeString(AppConfig.PREF_LOG_LEVEL) ?: "warning"
...
//            }
    ...
    private fun fakedns(v2rayConfig: V2rayConfig) {
        if (settingsStorage.decodeBool(AppConfig.PREF_LOCAL_DNS_ENABLED) && settingsStorage.decodeBool(AppConfig.PREF_FAKE_DNS_ENABLED)
        ) {
    ...
    private fun customLocalDns(v2rayConfig: V2rayConfig): Boolean {
        try {
            if (settingsStorage.decodeBool(AppConfig.PREF_FAKE_DNS_ENABLED)) {
        ...
        try {
            ...
            if (muxEnabled) {
                ...
                outbound.mux?.concurrency =
                    settingsStorage.decodeInt(AppConfig.PREF_MUX_CONCURRENCY)
                outbound.mux?.xudpConcurrency =
                    settingsStorage.decodeInt(AppConfig.PREF_MUX_XUDP_CONCURRENCY)
